# Architecture Index

This index lists all architecture documents in strict numerical order,
using **filenames** as the canonical identifiers.  
Human-readable titles are provided for clarity, but filenames govern structure.

---

## Foundations (01–07)

- **01-overview.md** — Overview  
- **02-pulse.md** — Pulse Architecture  
- **03-signal.md** — Signal Architecture  
- **04-aura.md** — Aura Architecture  
- **05-composer.md** — Composer Architecture  
- **06-processing-cohorts-and-anticipative-rendering.md** — Processing Cohorts & Anticipative Rendering  
- **07-project-versions-and-variants.md** — Project, Versions & Variants Architecture

---

## Structural Core (08–13)

- **08-tracks-lanes-and-roles.md** — Tracks, Lanes & Roles Architecture  
- **09-clips.md** — Clips Architecture  
- **10-parameters.md** — Parameters Architecture  
- **11-node-graph.md** — Node Graph Architecture  
- **12-mixer-and-channel-architecture.md** — Mixer & Channel Architecture  
- **13-media-architecture.md** — Media Architecture

---

## Timeline Core (14–19)

- **14-timebase-tempo-and-groove.md** — Timebase, Tempo & Groove Architecture  
- **15-advanced-clips.md** — Advanced Clips Architecture  
- **16-editing-and-nondestructive-layers.md** — Editing & Nondestructive Layers Architecture  
- **17-automation-and-modulation.md** *(planned)*  
- **18-midi-architecture.md** *(planned)*  
- **19-comping-architecture.md** *(planned)*

---

## Creative & Performance Systems (20–23)

- **20-clip-launcher.md** — Clip Launcher Architecture  
- **21-rendering-and-offline-processing.md** *(planned)*  
- **22-control-surfaces.md** *(planned)*  
- **23-ux-and-visual-layer.md** *(planned)*

---

## External Integration (24–27)

- **24-windowing-and-plugin-ui.md** — Windowing & Plugin UI Architecture  
- **25-diagnostics-and-performance.md** *(planned)*  
- **26-scripting-and-extensibility.md** *(planned)*  
- **27-video-architecture.md** *(planned)*

---

## Future Systems (28–31)

- **28-collaboration-architecture.md** *(future)*  
- **29-composer-extended-intelligence.md** *(future)*  
- **30-future-workflow-prototypes.md** *(future)*  
- **31-experimental-systems.md** *(future)*

---

## Notes

- Filename numbering is authoritative and must be maintained as the architecture expands.  
- Items marked **(planned)** are intentionally left for upcoming documentation passes.  
- Items marked **(future)** are long-range and do not constrain v1 implementation.
